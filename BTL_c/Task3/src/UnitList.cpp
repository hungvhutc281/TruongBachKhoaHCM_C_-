    ...
    else if (tong <= capacity)
    {
        if (unit != nullptr)
        {
            if (dynamic_cast<Vehicle*>(unit) != nullptr)
            {
                ...
                // Kiểm tra xem loại xe này đã tồn tại chưa
                node cur = head;
                while (cur != nullptr)
                {
                    if (Vehicle* UnitListVehicle = dynamic_cast<Vehicle*>(cur->unit))
                    {
                        ...
                        {
                            // Nếu trùng loại, cộng quantity và lấy weight lớn nhất
                ...
                // Đếm số loại xe hiện tại
                int currentVehicleTypes = 0;
                cur = head;
                while (cur != nullptr)
                {
                ...
                // Chỉ thêm xe mới nếu số loại xe hiện tại nhỏ hơn maxVehicleTypes
                if (currentVehicleTypes < maxVehicleTypes)
                {
